...
Object.assign(SERVICE_I18N.fr,{unblockPayout:"Lever le blocage",adminNewUser:"👤 {n} nouvel(le)(s) utilisateur(s)",online:"En ligne"});
Object.assign(SERVICE_I18N.en,{unblockPayout:"Remove hold",adminNewUser:"👤 {n} new user(s)",online:"Online"});
Object.assign(SERVICE_I18N.es,{unblockPayout:"Quitar bloqueo",adminNewUser:"👤 {n} nuevo(s) usuario(s)",online:"En línea"});
  ...
  connectInstancePromise=(async()=>{
    ...
    return sdk.init({publishableKey:STRIPE_PUBLISHABLE_KEY,locale:currentLang||'fr',
      fetchClientSecret:async()=>{
        const {data:{session:latest}}=await db.auth.getSession();
        if(generation!==connectGeneration || latest?.user.id!==owner){firstSecret=null;throw new Error(sf('sessionEnded'));}
...
async function renderServicePayoutAdmin(main,missions,byId){
  ...
  for(const m of rows){
    ...
    card.innerHTML=`<h3>${esc(m.port||"—")}</h3><p>${esc(pro?.full_name||sf("unknownPro"))} · ${esc(m.id.slice(0,8))}</p><strong>${esc(sf("netPro"))} : ${moneyC(Math.max(0,Number(m.amount_cents||0)+Number(m.urgent_fee_cents||0)-Number(m.platform_fee_cents||0)),m.currency||"eur")}</strong><p>${esc(c?serviceSummary(c,j,enabled):sf("legacyNoProof"))}</p><p>${blocked?esc(sf("blocked")):""} ${esc(account?.ready?sf("detailsReady"):sf("detailsIncomplete"))}</p><div class="request-actions"></div><p role="status">${esc(j?.error||"")}</p>`;
    ...
    if(c){
      button(sf("viewPhoto"),()=>showServiceProof(m.id));
      if(!j){
        button(c.held?sf("unblockPayout"):sf("blockPayout"),async()=>{await serviceRequest("hold",m.id,{held:!c.held});await renderAdminPanel("payouts");});
        button(sf("sendTransfer"),async()=>{if(!confirm(sf("confirmTransfer"))) return;await serviceRequest("transfer",m.id);await renderAdminPanel("payouts");},!enabled || blocked || c.held || !account?.ready);
      }
    }
    if(j && j.state!=="transferred") button(sf("verifyTransfer"),async()=>{await serviceRequest("reconcile",m.id);await renderAdminPanel("payouts");});
    main.append(card);
  }
}
...
async function enhanceAdminNotifications(){
  ...
  if(newRequests.length) panel.prepend(makeButton(sf("adminNewRequest",{n:newRequests.length}),"missions","requests",latestRequests));
  if(newUsers.length) panel.prepend(makeButton(sf("adminNewUser",{n:newUsers.length}),"users","users",latestUsers));

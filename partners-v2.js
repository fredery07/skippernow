(function(){
  ...
  let editingId = null;
  const TXT={
    fr:{partner:"Partenaire",sponsored:"Sponsorisé",discover:"Découvrir →",kicker:"Partenaires SkipperNow",title:"Nos partenaires",subtitle:"Ports, services nautiques et marques qui partagent notre univers.",prev:"Précédent",next:"Suivant",chooseImage:"Choisis une image.",imageTooLarge:"Image trop lourde : 5 Mo maximum.",visible:"● Visible",hidden:"● Masqué",order:"ordre",edit:"Modifier",hide:"Masquer",show:"Afficher",delete:"Supprimer",loading:"Chargement…",adminTitle:"Partenaires & publicités",adminSubtitle:"Ajoute, modifie et classe les partenaires affichés sur la page d’accueil.",viewHome:"Voir l’accueil ↗",name:"Nom / marque",subtitleLabel:"Sous-titre",subtitlePlaceholder:"Ex. Port partenaire à Cannes",link:"Lien",badge:"Badge",orderLabel:"Ordre",photo:"Photo",keep:"vide = conserver",visibleHome:"Visible sur l’accueil",sponsoredAd:"Sponsorisé / publicité",save:"Enregistrer",add:"Ajouter le partenaire",cancel:"Annuler",empty:"Aucun partenaire. Ajoute le premier ci-dessus.",confirmDelete:"Supprimer ce partenaire ?",saving:"Enregistrement…",addPhoto:"Ajoute une photo.",adminNav:"Partenaires & pubs"},
    en:{partner:"Partner",sponsored:"Sponsored",discover:"Discover →",kicker:"SkipperNow partners",title:"Our partners",subtitle:"Ports, marine services and brands that share our world.",prev:"Previous",next:"Next",chooseImage:"Choose an image.",imageTooLarge:"Image too large: 5 MB maximum.",visible:"● Visible",hidden:"● Hidden",order:"order",edit:"Edit",hide:"Hide",show:"Show",delete:"Delete",loading:"Loading…",adminTitle:"Partners & ads",adminSubtitle:"Add, edit and order the partners shown on the homepage.",viewHome:"View homepage ↗",name:"Name / brand",subtitleLabel:"Subtitle",subtitlePlaceholder:"E.g. Partner marina in Cannes",link:"Link",badge:"Badge",orderLabel:"Order",photo:"Photo",keep:"leave empty to keep",visibleHome:"Visible on homepage",sponsoredAd:"Sponsored / advertisement",save:"Save",add:"Add partner",cancel:"Cancel",empty:"No partners yet. Add the first one above.",confirmDelete:"Delete this partner?",saving:"Saving…",addPhoto:"Add a photo.",adminNav:"Partners & ads"},
    es:{partner:"Socio",sponsored:"Patrocinado",discover:"Descubrir →",kicker:"Socios de SkipperNow",title:"Nuestros socios",subtitle:"Puertos, servicios náuticos y marcas que comparten nuestro universo.",prev:"Anterior",next:"Siguiente",chooseImage:"Elige una imagen.",imageTooLarge:"Imagen demasiado pesada: máximo 5 MB.",visible:"● Visible",hidden:"● Oculto",order:"orden",edit:"Editar",hide:"Ocultar",show:"Mostrar",delete:"Eliminar",loading:"Cargando…",adminTitle:"Socios y publicidad",adminSubtitle:"Añade, edita y ordena los socios que aparecen en la página de inicio.",viewHome:"Ver inicio ↗",name:"Nombre / marca",subtitleLabel:"Subtítulo",subtitlePlaceholder:"Ej. Puerto asociado en Cannes",link:"Enlace",badge:"Etiqueta",orderLabel:"Orden",photo:"Foto",keep:"vacío = conservar",visibleHome:"Visible en inicio",sponsoredAd:"Patrocinado / publicidad",save:"Guardar",add:"Añadir socio",cancel:"Cancelar",empty:"Todavía no hay socios. Añade el primero arriba.",confirmDelete:"¿Eliminar este socio?",saving:"Guardando…",addPhoto:"Añade una foto.",adminNav:"Socios y publicidad"}
  };
  const tx=k=>(TXT[typeof currentLang!=="undefined"&&TXT[currentLang]?currentLang:"fr"]||TXT.fr)[k]||TXT.fr[k]||k;
  ...
  function card(p){
    const label=p.sponsored?tx("sponsored"):(p.badge||tx("partner"));
    const inner=`<img src="${esc(p.image_url)}" alt="${esc(p.name)}" loading="lazy"><span class="sn-p2-badge">${esc(label)}</span><span class="sn-p2-copy"><strong>${esc(p.name)}</strong>${p.subtitle?`<span>${esc(p.subtitle)}</span>`:""}${p.link_url?`<b>${esc(tx("discover"))}</b>`:""}</span>`;
  ...
  async function renderPublic(){
    ...
    sec.innerHTML=`<div class="sn-p2-head"><div><span class="sn-p2-kicker">${esc(tx("kicker"))}</span><h2>${esc(tx("title"))}</h2><p>${esc(tx("subtitle"))}</p></div><div class="sn-p2-nav"><button type="button" data-p2-prev aria-label="${esc(tx("prev"))}">←</button><button type="button" data-p2-next aria-label="${esc(tx("next"))}">→</button></div></div><div class="sn-p2-track">${rows.map(card).join("")}</div>`;
  ...
  async function upload(file){
    if(!file) return null; if(!file.type?.startsWith("image/"))throw new Error(tx("chooseImage")); if(file.size>5*1024*1024)throw new Error(tx("imageTooLarge"));
  ...
  function adminRow(p){
    return `<div class="sn-p2-row"><img src="${esc(p.image_url)}" alt=""><div><strong>${esc(p.name)}</strong><div class="muted">${esc(p.subtitle||"")}</div><div class="sn-p2-status ${p.is_active?"":"off"}">${esc(p.is_active?tx("visible"):tx("hidden"))}${p.sponsored?" · "+esc(tx("sponsored")):""} · ${esc(tx("order"))} ${Number(p.sort_order||0)}</div></div><div class="sn-p2-actions"><button class="small-btn" type="button" data-p2-edit="${p.id}">${esc(tx("edit"))}</button><button class="small-btn" type="button" data-p2-toggle="${p.id}">${esc(p.is_active?tx("hide"):tx("show"))}</button><button class="small-btn danger" type="button" data-p2-delete="${p.id}">${esc(tx("delete"))}</button></div></div>`;
  }

  async function renderAdmin(){
    const main=document.querySelector("#dashMain"); if(!main)return;
    main.innerHTML=`<div class="empty-note">${esc(tx("loading"))}</div>`;
    ...
    document.querySelectorAll("[data-p2-delete]").forEach(b=>b.onclick=async()=>{if(!confirm(tx("confirmDelete")))return;const{error}=await getDb().from("home_partners").delete().eq("id",b.dataset.p2Delete);if(error)alert(error.message);else{editingId=null;renderAdmin();}});
    document.querySelector("#p2Form").onsubmit=async e=>{e.preventDefault();const msg=document.querySelector("#p2Msg"),submit=e.target.querySelector('button[type="submit"]');submit.disabled=true;msg.textContent=tx("saving");try{const file=document.querySelector("#p2Image").files?.[0],image=(await upload(file))||p?.image_url;if(!image)throw new Error(tx("addPhoto"));const payload={name:document.querySelector("#p2Name").value.trim(),subtitle:document.querySelector("#p2Subtitle").value.trim()||null,link_url:document.querySelector("#p2Link").value.trim()||null,badge:document.querySelector("#p2Badge").value.trim()||"Partenaire",sort_order:Number(document.querySelector("#p2Order").value||100),is_active:document.querySelector("#p2Active").checked,sponsored:document.querySelector("#p2Sponsored").checked,image_url:image};const q=p?getDb().from("home_partners").update(payload).eq("id",p.id):getDb().from("home_partners").insert(payload);const{error}=await q;if(error)throw error;editingId=null;await renderAdmin();}catch(err){msg.style.color="#a22e2e";msg.textContent=err.message||String(err);submit.disabled=false;}};
  }
  ...
  async function installAdminButton(){
    ...
    const btn=document.createElement("button");btn.type="button";btn.dataset.p2Admin="1";btn.dataset.panel="partners";btn.innerHTML=`🤝 <span>${esc(tx("adminNav"))}</span>`;btn.onclick=async()=>{side.querySelectorAll("button").forEach(x=>x.classList.toggle("active",x===btn));await renderAdmin();};side.appendChild(btn);
  }

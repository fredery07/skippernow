...
(()=>{
  ...
  async function request(action,missionId,extra={}){
    ...
    if(seq!==generation || owner!==requestOwner)throw Error(L().sessionChanged);
  ...
  window.openInvoice=async function(missionId){
    ...
    root.innerHTML=`<div class="dialog-top"><h2>${esc(l.title)}</h2><button class="close" type="button" aria-label="${esc(l.close)}" onclick="closeModal('invoiceModal')">×</button></div><div data-doc-body role="status">${esc(l.loading)}</div>`;

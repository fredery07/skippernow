(function(){
  ...
  async function renderRentalPanel(){
    ...
    body.innerHTML=`<div class="panel-head"><div><h3>Locations de bateaux</h3><p class="muted">Vos demandes envoyées et les demandes reçues pour vos bateaux.</p></div></div>
      <div class="request-list">${rows.length?rows.map(r=>{
        ...
        if(!isOwner && r.status==="accepted" && r.payment_status==="unpaid") actions+=` <span class="tag">Paiement sécurisé : étape suivante</span>`;
  ...
  function installDashboardEntry(){
    ...
    btn.type="button";
    btn.dataset.panel="boat-rentals";
    btn.innerHTML="🚤 <span>Locations bateaux</span>";
    btn.addEventListener("click",async()=>{
      side.querySelectorAll("button").forEach(b=>b.classList.toggle("active",b===btn));
      await renderRentalPanel();
    });
  ...
  function install(){
    ...
    if(submit) submit.addEventListener("click",async function(ev){
      ...
      const hours=duration==="half"?4:duration==="day"?8:days*24;
      ...
      const {error}=await db.from("boat_rental_requests").insert({
      ...
      document.querySelector("#bookingSuccessText")?.replaceChildren(document.createTextNode("Le propriétaire a reçu votre demande de location."));
    },true);

    const originalOpenDashboard=window.openDashboard;
    if(typeof originalOpenDashboard==="function") window.openDashboard=async function(){

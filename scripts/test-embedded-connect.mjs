...
function database(initial={}){
  let row={professional_id:'pro',account_id:null,creation_key:'stable-key',account_api:'v1',embedded_started_at:null,...initial};
...
const payload=c.accountPayload('pro','FR');assert.equal(payload.dashboard,'none');assert.equal(payload.identity.country,'fr');assert.equal(payload.configuration.merchant,undefined);
...
await c.ensureAccount(state.db,'pro','FR',api);await c.ensureAccount(state.db,'pro','US',api);
...
await assert.rejects(c.ensureAccount(state.db,'pro','FR',interrupted),/network/);
await c.ensureAccount(state.db,'pro','US',interrupted);assert.deepEqual(creates[0],creates[1]);
// A parallel first click cannot change the stored creation payload.
state=database();creates=[];await Promise.all([c.ensureAccount(state.db,'pro','FR',api),c.ensureAccount(state.db,'pro','US',api)]);assert.ok(creates.every(x=>JSON.stringify(x)===JSON.stringify(creates[0])));
// A previously created account is recovered without creating a second one.
state=database();let calls=0;
await c.ensureAccount(state.db,'pro','FR',async(path,body)=>{assert.equal(body,undefined);calls++;return {data:[{id:'acct_old',metadata:{professional_id:'pro'}}],has_more:false};});assert.equal(state.get().account_id,'acct_old');assert.equal(calls,1);
...
await assert.rejects(c.ensureAccount(state.db,'pro','FR',async(path,body)=>{assert.equal(body,undefined);return {data:[],has_more:false};}),/support/);

...
else if(payload.action==='receipt')data={receipt:{reference:'SN-ch_paid',paymentReference:'pi_paid',paidAt:'2026-09-10T10:00:00Z',total:6000,refunded:1000,currency:'eur',missionId:'mission',port:'Cannes',description:'Nettoyage <img src=x onerror=alert(1)>'}};
...
const button=document.querySelector('[data-receipt-load]');button.click();await new Promise(r=>setImmediate(r));assert.ok(document.body.textContent.includes('60,00'));assert.ok(document.body.textContent.includes('10,00'));assert.equal(document.querySelector('[data-receipt] img'),null);assert.ok(!document.body.textContent.includes('Net professionnel'));
